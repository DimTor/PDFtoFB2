...
def margins(margin: str) -> list:
    '''
    В случае, если отступы заданы в виде строки, то превращает из в список

    Args:
        margin(str): Строка с отступами
    '''
...
def parse_arguments():
    """
    Сбор данных из командной строки
    """
    ...
    parser.add_argument(
        "--output_dir", type=str, nargs="?", help="The output directory", default="out_form"
    )
    parser.add_argument(
        "-i",
        "--input_file",
...
def main():
    """
    Проходит сбор всех данных, полученных из командной строки и формирование на их основе параметров для генерации
    """
    # Argument parsing
    start_time = time.time()
    args = parse_arguments()
    train_size = 10 * args.count
    test_size = 1 * args.count
    val_size = 1 * args.count
    ...
    fonts = load_fonts('for_formul')
    # Creating synthetic sentences (or word)
    for i in mode:
        strings = []
        try:
            os.makedirs(args.output_dir + '/images/' + i)
        except OSError as e:
            if e.errno != errno.EEXIST:
                raise
        try:
            os.makedirs(args.output_dir + '/labels/' + i)
        except OSError as e:
            if e.errno != errno.EEXIST:
                raise
        if args.use_wikipedia:
            ...
            strings = create_strings_from_dict(
                args.length, args.random, mode[i], lang_dict
            )
        ...
        for _ in tqdm(
            p.imap_unordered(
                ...
                zip(
                    ...
                    [args.output_dir + '/images/' + i] * string_count,
                    form,
